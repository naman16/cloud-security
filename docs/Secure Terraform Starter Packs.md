...
* _Paved Roads_: Originally conceptualized by Netflix, refers to a set of standardized frameworks (e.g., authentication patterns, certificate management, service mesh, etc.), self-service tools, and automated processes that allow engineers to focus on their core responsibilities, without worrying about security.  
* _Security Guardrails_: These are preventive controls, integrated into the development workflows, that define the security boundaries and force the engineers to operate with them, thereby stopping misconfigured and vulnerable resources (code) from being released into cloud environments. 

Most organizations have some level of implementation for paved roads and / or security guardrails, albeit at varying levels of maturity. In my experience, the focus has been primarily geared towards a) enabling scanning / detection as early in the SDLC as possible and b) ad-how prevention of limited high-risk actions via CI / CD pipeline integrations and organizational policies (e.g., Service Control Policies (SCPs), Organization Policies, Azure Policies). 
...
* Security teams only want to prescribe requirements, run scans, and report on issues; there’s little effort spent in getting hands-on with the engineers to develop reusable secure artifacts \- “build once, use multiple times”.  
...
I have been thinking about this problem statement for the last year now and have been curious about using large language models (LLMs) to develop secure and reusable IaC. I tried implementing this a few times before but never got satisfactory results for various reasons \- poor approach, broader definition of the problem statement, LLM performance, etc. But in my most recent attempt, the results were impressive. Below, I have shared the results and the source code for anyone to use as well as provided some thoughts on how it can be expanded to generate IaC for different languages, cloud providers, and requirement sets.
...
Once the Terraform modules were generated, I was naturally very excited to test them for accuracy and security. Below are the high-level results from some lightweight testing. 
...
* Some of the issues (e.g., latest version mismatches, missing encryption-at-rest, use of default security groups etc.) were genuine and I tried addressing the majority of them.
